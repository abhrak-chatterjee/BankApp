...
//		Query query= session.createQuery("from bankdetails");
//		ArrayList<SavingAccount> empList= (ArrayList<SavingAccount>) query.list();
//		transaction.commit();
//		session.close();
//		return empList;
//	}
	public List<SavingAccount> retrieve()
	{
		Session session = this.sessionFactory.openSession();
        Transaction t = session.beginTransaction();  
        @SuppressWarnings("unchecked")
        List<SavingAccount> accountList = session.createQuery("from BankAccount").list();
        t.commit(); 
        session.close();
        return accountList;
